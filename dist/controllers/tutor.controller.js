...
exports.tutorController = {
    ...
    async getAll(req, res) {
        try {
            const tutors = await tutor_repository_1.tutorRepository.findAll();
            if (tutors && tutors.length > 0) {
                ...
                tutors.forEach((tutor) => {
                    ...
                    const tutorCourses = courseMap.filter((c) => c.tutor_id === tutor.tutor_id);
                    tutor.total_courses = tutorCourses.length;
                    // Danh sách các course_id của giảng viên này
                    const tutorCourseIds = new Set(tutorCourses.map((c) => c.course_id));
                    // Đọc danh sách học viên đăng ký các khóa học này
                    const tutorBookings = bookingMap.filter((b) => tutorCourseIds.has(b.course_id));
                    // Số học viên độc nhất (unique student_id)
                    tutor.total_students = new Set(tutorBookings.map((b) => b.student_id)).size;
                });
            }
    ...
    async getById(req, res) {
        try {
            ...
            const tutor = await tutor_repository_1.tutorRepository.findById(tutorId);
            if (!tutor) {
            ...
            if (tutorCourses.length > 0) {
                ...
                tutor.total_students = new Set((bookings || []).map((b) => b.student_id)).size;
            }
            else {
    ...
    async getStats(req, res) {
        try {
            ...
            const data = await tutor_service_1.tutorService.getStats(userId);
    ...
    async getBookings(req, res) {
        try {
            ...
            const data = await tutor_service_1.tutorService.getBookings(userId);
            res.status(200).json({ success: true, data });
        }
        catch (error) {
    ...
    async updateBookingStatus(req, res) {
        try {
            ...
            const data = await tutor_service_1.tutorService.updateBookingStatus(userId, bookingId, status);
    ...
    async getReviews(req, res) {
        try {
            ...
            const data = await tutor_service_1.tutorService.getReviews(userId);
            res.status(200).json({ success: true, data });
        }
        catch (error) {
    ...
    async getWallet(req, res) {
        try {
            ...
            const data = await tutor_service_1.tutorService.getWallet(userId);
            res.status(200).json({ success: true, data });
        }
        catch (error) {
    ...
    async withdrawFunds(req, res) {
        try {
            ...
            const data = await tutor_service_1.tutorService.withdrawFunds(userId, Number(amount), bankName, bankAccount);
    ...
    async getMyProfile(req, res) {
        try {
            ...
            const data = await tutor_service_1.tutorService.getMyProfile(userId);
            res.status(200).json({ success: true, data });
        }
        catch (error) {
    ...
    async updateMyProfile(req, res) {
        try {
            ...
            const data = await tutor_service_1.tutorService.updateMyProfile(userId, req.body);
    ...
    async addCertificate(req, res) {
        try {
            ...
            const data = await tutor_service_1.tutorService.addCertificate(userId, req.body);
    ...
    async deleteCertificate(req, res) {
        try {
            ...
            await tutor_service_1.tutorService.deleteCertificate(userId, certId);
    ...
    async getClassSessions(req, res) {
        try {
            ...
            const result = await tutor_service_1.tutorService.getClassSessions(userId);
