...
| **05** | **Class Session & Attendance** | Quản lý từng buổi học thực tế, điểm danh và ghi âm/băng hình buổi học. | `class_sessions`, `attendances`, `session_recordings`<br>*ENUMs:* `SessionStatus`, `AttendanceStatus`, `AttendanceMethod` | *(Đang cập nhật)* |
| **06** | **Chat & Messaging** | Quản lý phòng chat 1-1, chat nhóm và tin nhắn trao đổi giữa Học sinh & Gia sư. | `chat_rooms`, `messages`<br>*ENUMs:* `MessageType`, `RoomType` | *(Đang cập nhật)* |
| **07** | **Review, Favorite & Matching** | Quản lý đánh giá gia sư, danh sách gia sư yêu thích và log đề xuất từ thuật toán AI. | `reviews`, `favorites`, `matching_logs` | *(Đang cập nhật)* |
| **08** | **Document & AI** | Quản lý tài liệu khóa học, trích xuất Vector Search (pgvector) và lịch sử chat AI. | `documents`, `document_chunks`, `ai_conversations` | *(Đang cập nhật)* |
| **09** | **Quiz** | Quản lý ngân hàng câu hỏi trắc nghiệm/tự luận, tạo đề thi AI và kết quả làm bài. | `quizzes`, `quiz_questions`, `quiz_attempts`<br>*ENUMs:* `QuizQuestionType`, `QuizStatus` | *(Đang cập nhật)* |
| **10** | **Admin & System** | Quản lý nhật ký hoạt động Admin, cấu hình hệ thống và trạng thái bảng vẽ chung. | `admin_logs`, `system_configs`, `whiteboard_states` | *(Đang cập nhật)* |
| **11** | **Article & Comment** | Quản lý tin tức, bài viết kinh nghiệm học tập và các bình luận trao đổi. | `articles`, `article_comments`, `course_comments` | *(Đang cập nhật)* |
